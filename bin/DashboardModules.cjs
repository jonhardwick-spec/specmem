...
/**
 * ClaudePreviewModule - TOP LEFT: Live Claude screen preview
 */
class ClaudePreviewModule extends DashboardModule {
  constructor(renderer, options = {}) {
    super('ClaudePreview', renderer, options);
    this.screenCache = '';
    this.lastContent = '';
  ...
  async update() {
    ...
    try {
      ...
      if (screenContent.trim()) {
        ...
        // Empty state
        this.lines.push('');
        this.lines.push(`${c.dim || ''}  No Claude session output${c.reset || ''}`);
        this.lines.push('');
        if (!activeSession) {
  ...
  async update() {
    ...
    try {
      ...
      if (cotContent && cotContent.length > 0) {
        ...
        this.lines.push(`${c.dim || ''}Claude is thinking...${c.reset || ''}`);
      }
    } catch (err) {
      this.lines.push(`${c.red || ''}[Error: ${err.message}]${c.reset || ''}`);
    }
    ...
    try {
      ...
      if (!hasDbCalls) {
        ...
        if (history && history.length > 0) {
          ...
          this.lines.push('');
          this.lines.push(`${c.dim || ''}Tools appear here when Claude${c.reset || ''}`);
  ...
  async update() {
    ...
    try {
      ...
      if (inputMode === 'claude') {
        ...
        this.lines.push('');
        this.lines.push(`${c.cyan || ''}>${c.reset || ''} ${c.dim || ''}[typing directly into Claude]${c.reset || ''}`);
        this.lines.push('');
        this.lines.push(`${c.dim || ''}All keystrokes sent directly to Claude${c.reset || ''}`);
        ...
        this.lines.push(`  ${c.brightCyan || ''}i${c.reset || ''} ${c.dim || ''}-${c.reset || ''} Chat with Claude`);
