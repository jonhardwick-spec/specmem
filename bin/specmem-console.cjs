...
`;
  ...
  enterDashboardMode() {
    ...
    const draw = async () => {
      ...
      // Format: "SPECMEM DASHBOARD - {project} |  {n} "alias" [total] | {COMMAND/CLAUDE/SPECMEM} | {agents}"
      ...
      if (inputMode === 'claude' || inputMode === 'specmem') {
        ...
        footer = ` ${modeIcon} ${c.dim}|${c.reset} Ctrl+Shift+${icons.arrow_u}${icons.arrow_d}=Mode ${c.dim}|${c.reset} Ctrl+Shift+${icons.arrow_l}${icons.arrow_r}= ${c.dim}|${c.reset} Ctrl+Shift+H=Exit`;
      } else {
        // Command mode - show essential shortcuts only
        footer = ` ${modeIcon} ${c.dim}|${c.reset} Ctrl+Shift+${icons.arrow_u}${icons.arrow_d}=Mode ${c.dim}|${c.reset} Ctrl+Shift+${icons.arrow_l}${icons.arrow_r}= ${c.dim}|${c.reset} ${c.white}n${c.reset}=New ${c.dim}|${c.reset} ${c.white}h${c.reset}=Help`;
      }
      frame += `\n${footer}`;
    ...
    const keyHandler = (key) => {
      ...
      if (key === 'q' || key === 'Q' || key === '\x03' || key === '\x1b[67;6u' || key === '\x1b[99;6u') {
        ...
        // Number keys 1-9 to switch to  session by number
