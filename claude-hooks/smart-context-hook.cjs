...
const { Pool } = require('pg');  // ASYNC pg Pool - no more blocking execSync!
...
/**
 * Get schema name from project path (matches projectNamespacing.ts pattern)
 */
...
const CONFIG = {
  ...
  dbName: expandCwd(process.env.SPECMEM_DB_NAME) || 'specmem',
  dbUser: expandCwd(process.env.SPECMEM_DB_USER) || 'specmem',
  dbPassword: expandCwd(process.env.SPECMEM_DB_PASSWORD) || 'specmem'
};
...
async function generateEmbedding(text) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let buffer = '';
    socket.setTimeout(5000);  // Reduced timeout from 10s to 5s for faster failure
