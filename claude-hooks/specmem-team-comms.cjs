...
// Use shared specmem-paths for pg classes
const { getPool, getClient: getPgClient, getSchemaName } = require('./specmem-paths.cjs');
const Pool = getPool();
const Client = getPgClient();

// Exit early if pg not available
if (!Pool || !Client) {
  console.error('pg module not available - cannot connect to database');
  process.exit(1);
}
