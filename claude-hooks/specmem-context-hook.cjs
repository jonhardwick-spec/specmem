...
// Import shared path resolution utilities AND pg classes
const {
  expandCwd,
  getSpecmemPkg,
  getSpecmemHome,
...
// Get pg classes (or null if pg not available)
const Pool = getPool();
...
const CONFIG = {
  db: {
    ...
    database: expandCwd(process.env.SPECMEM_DB_NAME) || unifiedCred,
    user: expandCwd(process.env.SPECMEM_DB_USER) || unifiedCred,
    password: expandCwd(process.env.SPECMEM_DB_PASSWORD) || unifiedCred
  },
