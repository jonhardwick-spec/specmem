 ...
 */
function connectWithFailFast(socketPath, timeoutMs = 5000) {
 ...
 */
async function sendMessage(socketPath, message, timeoutMs = 5000) {
  let socket = null;
  let settled = false;
  let buffer = '';
 ...
 */
async function generateEmbedding(socketPath, text, timeoutMs = 10000) {
  const response = await sendMessage(
    socketPath,
    { type: 'embed', text },
    timeoutMs
