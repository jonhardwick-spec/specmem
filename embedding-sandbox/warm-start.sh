...
# RELIABILITY FIX: Check if socket file exists (server started but may be warming up)
# Use this for fast startup detection - server created socket but may still be loading model
socket_exists() {
    [ -S "$SOCKET_PATH" ]
}
...
# RELIABILITY FIX: Two-phase wait - first for socket existence, then for responsiveness
# This prevents premature timeout when server is loading model
wait_for_socket() {
    log "Waiting for socket..."

    # Phase 1: Wait for socket file to exist (up to 30s with fast polling)
    # Server creates socket BEFORE loading model, so this is quick
    log "Phase 1: Waiting for socket file..."
    for i in {1..60}; do
        if socket_exists; then
            log "Socket file appeared after ${i}*0.5s"
            break
        fi
        sleep 0.5
    done
    ...
    if ! socket_exists; then
        log "ERROR: Socket file never appeared"
        return 1
    fi

    # Phase 2: Wait for socket to respond (up to 90s for model warmup)
    # First-time model loading can take 20-30s, give plenty of time
    log "Phase 2: Waiting for socket to respond..."
    for i in {1..90}; do
        if socket_alive; then
            log "Socket responding after ${i}s"
            return 0
        fi
        sleep 1
    done

    log "ERROR: Socket exists but never became responsive (model loading may have failed)"
    return 1
}
