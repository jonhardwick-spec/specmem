...
const TIMEOUTS = {
  ...
  EMBEDDING_WARMUP: 15000,      // 15s - initial warmup request (cold model)
  EMBEDDING_REQUEST: 60000,     // 60s - standard embedding generation
  EMBEDDING_BATCH: 30000,       // 30s - batch embedding operations

  // Connection testing
  CONNECTION_TEST: 10000,       // 10s - quick socket/db connection checks
...
async function coldStartEmbeddingDocker(projectPath, modelConfig, ui, codebaseResult) {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    if (i % 10 === 0) {
      ui.setSubProgress(0.3 + (i / 120) * 0.4); // Progress from 30% to 70%
    }
  }
...
async function indexCodebase(projectPath, ui, embeddingResult) {
  ...
  if (!serverRunning && !socketExists) {
    ...
    if (embeddingScript) {
      ...
      if (!activeSocketPath) {
        ui.setSubStatus('⚠️ Embedding server failed to start - continuing without');
      }
    } else {
    ...
    try {
      ...
      if (fs.existsSync(warmStartScript)) {
        ...
        try {
          ...
          // Wait for Docker to start (up to 45s for recovery)
  ...
  async function generateEmbedding(text) {
      ...
      try {
        const result = await new Promise((resolve, reject) => {
          ...
          client.connect(activeSocketPath, () => {
            // yooo type:'embed' is required - server expects this format fr
            client.write(JSON.stringify({ type: 'embed', text: text.slice(0, MAX_EMBED_CHARS) }) + '\n');
          });

          client.on('data', chunk => {
            data += chunk.toString();
            // yooo TCP can split responses - process ALL complete lines before newline
            // Server sends heartbeat {"status":"processing"} FIRST, then actual embedding
            let newlineIdx;
            while ((newlineIdx = data.indexOf('\n')) !== -1) {
              const completeLine = data.slice(0, newlineIdx);
              ...
              try {
                ...
                // yooo skip heartbeat/processing status - keep waiting for actual embedding
                if (parsed.status === 'processing') {
                  continue; // wait for next message
                }

                client.destroy();
                // yooo check for error response FIRST before dangerous fallback
                if (parsed.error) {
                ...
                // yooo validate embedding has reasonable dimensions (32-4096)
        ...
        lastError = err;
        // yooo retry logic kicking in, exponential backoff no cap
    ...
    // all retries cooked fr - silent to not break ProgressUI
  ...
  function updateProgress() {
    ...
    if (now - lastUIUpdate > 100 || processed >= files.length) {
      ...
      ui.setSubStatus(`${processed}/${files.length} files - ${results.embeddingsGenerated} embeddings${failStr}`);
    }
  }
  ...
  for (let i = 0; i < files.length; i += batchSize) {
    ...
    // Log progress every 10 batches (500 files with batchSize=50)
    if ((i / batchSize) % 10 === 0) {
      initLog(`Batch progress: ${i}/${files.length} files, ${results.embeddingsGenerated} embeddings, ${results.embeddingsFailed || 0} failed`);
    }

    // parallel process files within batch
    await runWithConcurrency(batch, async (filePath) => {
      try {
        ...
        // yooo use language from fileLanguageMap (populated by bridge or fallback)
        const langInfo = fileLanguageMap.get(filePath);
        const language = langInfo ? langInfo.id : 'unknown';
        const lineCount = content.split('\n').length;

        // Generate embedding (only if socket available)
        let embedding = null;
        try {
          ...
          if (embedding && Array.isArray(embedding)) {
            ...
            // Track why embedding was null - helps debug dimension mismatches
            results.embeddingsSkipped = (results.embeddingsSkipped || 0) + 1;
          }
        } catch (e) {
          // Track embedding failures (not just silent skip)
          results.embeddingsFailed = (results.embeddingsFailed || 0) + 1;
          // Log first 5 failures for debugging
          if (results.embeddingsFailed <= 5) {
            initLog(`Embedding failed for ${relativePath}: ${e.message}`);
          }
        }

        // validate embedding is array before .join() - generateEmbedding can return non-array
        const embeddingStr = (embedding && Array.isArray(embedding) && embedding.length > 0)
          ? '[' + embedding.join(',') + ']'
          : null;
        const fileId = uuidv4();

        // Insert or update file using DELETE + INSERT for safer upsert
        // (avoids unique constraint issues across different schema versions)
        try {
          await pool.query(
          ...
          // Ignore delete errors (table might not have project_path)
        }
        ...
        `, [
          fileId, relativePath, filePath, fileName, ext,
          language.toLowerCase(), language, content, contentHash,
          stats.size, lineCount, embeddingStr, projectPath
        ]);

        results.filesIndexed++;

        // Extract definitions (functions, classes) - also parallelize these embeddings
        // Limit definitions per file to prevent explosion (some files have 100+ defs)
        const definitions = await extractDefinitions(content, relativePath, language, fileId);
        const maxDefsPerFile = 30; // cap to prevent slowdown on large files
        const defsToProcess = definitions.slice(0, maxDefsPerFile);
        if (defsToProcess.length > 0) {
          await runWithConcurrency(defsToProcess, async (def) => {
            try {
              // Generate embedding for definition
              let defEmbedding = null;
              try {
                const defText = def.type + ' ' + def.name + '\n' + (def.signature || '') + '\nFile: ' + relativePath;
                defEmbedding = await generateEmbedding(defText);
              } catch (e) {
                // yooo def embedding failed - storing without it fr
                if (process.env.SPECMEM_DEBUG) console.log('[INDEX] def embedding failed for ' + def.name + ': ' + e.message);
              }

              // validate defEmbedding is array before .join()
              ...
              await pool.query(
                ...
                'ON CONFLICT (id) DO UPDATE SET ' +
                  ...
                  'updated_at = NOW()',
                [
                  def.id, fileId, relativePath, def.name, def.type,
                  def.startLine, def.endLine, def.signature, 'public',
                  true, projectPath, defEmbeddingStr
                ]
              );
              results.definitionsExtracted++;
            } catch (e) {
              // yooo def insert failed - db might be cooked or constraint violated
    ...
    }, parallelLimit, updateProgress);  // Per-embed progress callback - updates UI as each file completes
...
async function extractSessions(projectPath, ui, embeddingResult = null) {
  ...
  async function genEmbed(text) {
    ...
    return new Promise((resolve, reject) => {
      const client = new net.Socket();
      let data = '';
      client.setTimeout(TIMEOUTS.EMBEDDING_BATCH);
      client.connect(socketPath, () => {
        // yooo MUST include type:'embed' - server requires this format fr
        client.write(JSON.stringify({ type: 'embed', text: text.slice(0, MAX_EMBED_CHARS) }) + '\n');
      });
      client.on('data', chunk => {
        data += chunk.toString();
        // yooo TCP splits responses - process ALL complete lines
        // Server sends heartbeat {"status":"processing"} FIRST, then actual embedding
        let newlineIdx;
        while ((newlineIdx = data.indexOf('\n')) !== -1) {
          const completeLine = data.slice(0, newlineIdx);
          ...
          try {
            ...
            const embedding = Array.isArray(parsed.embedding) ? parsed.embedding :
                              Array.isArray(parsed) ? parsed : null;
            if (!embedding) {
              reject(new Error('Invalid embedding response: expected array'));
              return;
            }
            // yooo validate 384 dimensions - model should always return this size fr
            if (embedding.length !== 384) {
              uiWarn('Embedding wrong dims: ' + embedding.length + ' (expected 384)');
              reject(new Error('Invalid embedding dimensions: ' + embedding.length));
              return;
            }
            resolve(embedding);
            return; // done processing
          } catch (parseErr) { client.destroy(); reject(parseErr); return; }
        }
      });
  ...
  for (let i = 0; i < newEntries.length; i += BATCH_SIZE) {
    ...
    for (const entry of batch) {
      try {
        const embedding = await genEmbed(entry.formatted);
        const tags = [
        ...
        // validate embedding before .join() - genEmbed can return undefined
        const embeddingStr = (embedding && Array.isArray(embedding) && embedding.length > 0)
          ? '[' + embedding.join(',') + ']'
          : null;
        await pool.query(`
      ...
      updateEntryProgress();  // Update progress after each entry
    }
    await qqms();
  }
...
async function verifySetupPrerequisites(projectPath) {
  ...
  checks.claudeHooks = fs.existsSync(hooksDir);
...
async function runAutoSetup(projectPath) {
  ...
  try {
    ...
    setupUI.setSubProgress(1);
    setupUI.setSubStatus(`✓ ${hookCount} hooks installed`);
    await qqms();
