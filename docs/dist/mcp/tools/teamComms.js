...
- "Completed the database migration, moving to testing" (update)`;
    ...
    async execute(params) {
        ...
        const projectPath = getProjectPathForInsert();
        const projectHash = require('crypto').createHash('sha256').update(projectPath).digest('hex').slice(0, 12);
    ...
    async execute(params) {
        ...
        if (isDBAvailable()) {
            ...
            try {
                ...
                const channelIds = [getProjectDefaultChannelId()];
        ...
        `;
          ...
          `, [memberId, messageIds]);
            ...
            let memMessages = Array.from(teamMessagesMemory.values())
                .filter(msg => msg.channel_id === getProjectDefaultChannelId())
