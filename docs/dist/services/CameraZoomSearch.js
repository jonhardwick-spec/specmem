...
options) {
    ...
    try {
        ...
        if (entry.type === 'code') {
            logger.debug({ drilldownID, memoryID }, '[Drilldown] Code type - querying code_definitions');
      ...
      `, [memoryID]);
            if (codeResult.rows.length === 0) {
                logger.warn({ memoryID }, '[Drilldown] Code definition not found');
                return null;
            }
            const code = codeResult.rows[0];
            // Build human-readable code content
            const codeContent = [
                `[${code.definition_type.toUpperCase()}] ${code.name}`,
                `File: ${code.file_path}:${code.start_line}-${code.end_line}`,
                `Language: ${code.language}${code.is_exported ? ' (exported)' : ''}`,
                '',
                code.signature || '',
                '',
                code.content || ''
            ].filter(Boolean).join('\n');
            // Return simplified result for code (matches DrilldownResult interface)
            return {
