...
const CONFIG = {
  ...
  threshold: parseFloat(process.env.SPECMEM_THRESHOLD || '0.3'),
...
function formatMemories(memories) {
  ...
  sorted.forEach((mem, i) => {
    ...
    // Use numeric index as drilldownID (drill_down resolves to UUID internally)
    const drilldownId = i + 1;

    // Format: N.[sim%] content [tags]
    parts.push(drilldownId + '.[' + sim + '] ' + formattedContent + ' ' + tags);
  });

  // Consistent closing tag + drilldown instruction
  parts.push('[/SM-找] drill_down(1|2|3...) 滿① 內容');
