...
function isAgentSession(sessionId) {
  ...
  try {
    if (!fs.existsSync(ACTIVE_AGENTS_FILE)) return false;
    const agents = JSON.parse(fs.readFileSync(ACTIVE_AGENTS_FILE, 'utf8'));
    // Check if this sessionId matches any registered agent
  ...
  try {
    ...
    if (!state.announced) {
      ...
      console.log(blockResponse(
        ...
        `REQUIRED: send_team_message({channel:"swarm-1", message:"Starting: [your task]", type:"status"})\n\n` +
        `Then you can proceed with other tools.`
      ));
      return;
    }
